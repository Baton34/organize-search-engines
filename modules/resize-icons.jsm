...
Resizer.prototype = {
  ...
  get _canvas() {
    if(!this.__canvas)
      this.__canvas = this._createCanvas();
    return this.__canvas;
  },
  __document: null,
  get _document() {
    if(!this.__document)
      this.__document = Services.wm.getMostRecentWindow("navigator:browser").document;
  ...
  addIconByImage: function(img) {
    if(!img.naturalWidth && !img.getContext) { // not loaded yet
      this._addLoadingIcon(img);
    } else {
      this.icons.push(img);
      this._iconAdded(img);
